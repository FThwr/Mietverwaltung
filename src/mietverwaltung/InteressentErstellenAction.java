...
public class InteressentErstellenAction extends MenueManager implements Action {
		...
		while (erstellVorgang == true) {
			...
			try {
				...
				if (änderung == 8) {
					...
					erstellVorgang = false;
					interestList.add(
							new Interessent(interessentenID, name, vorname, geburtsdatum, EMail, adresse, telefonnummer, status));
				}
			} catch (InputMismatchException e) {
				System.out.println("\n------------------------------- Fehler! ------------------------------- \nSie haben einen Buchstaben eingegeben, wo eine Zahl erwartet wurde!\n");
			}
