	...
	public void action() {
		...
		try {
			...
			String[] buttons = { "Wohnungsnummer", "Zimmeranzahl", "Fläche", "Kosten", "Etage", "Balkon",
					"Fußbodenheizung", "Aussicht", "Adresse", "letztes Renovierungsdatum",
					"Renovierungsanzahl", "letztes Renovierungsdetails", "zugeordneten Mitarbeiter"};
				...
				while (bearbeitungsVorgang == true) {
					...
					try {
						...
						if (ok == 1) {
							...
							if (nichtVorhanden == 1) {
								System.out.println("Die Wohnung mit der Nummer: " + "'" + neu + "'"
											+ " existiert nicht!\n");
									neu = neueID;
							}
							if (nichtVorhanden == 2) {
